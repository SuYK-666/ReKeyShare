...
public final class AuditProofService {
	public record AuditProof(Instant generatedAt, int eventCount, String chainRoot, String merkleRoot,
	...
	/**
	 * Compatibility constructor for local fixtures. A fresh asymmetric signing key
	 * is generated; the value is deliberately not used as a shared signing secret.
	 */
	...
	public boolean verifyProof(AuditProof proof) {
		try {
			if (!"Ed25519".equals(proof.signatureAlgorithm()) || !java.security.MessageDigest
					.isEqual(verificationKey.getEncoded(), Base64.getDecoder().decode(proof.publicKey()))) {
				return false;
			}
	...
	public String exportJson(AuditProof proof) {
		return "{" + "\"generatedAt\":\"" + proof.generatedAt() + "\"," + "\"eventCount\":" + proof.eventCount() + ","
				+ "\"chainRoot\":\"" + proof.chainRoot() + "\"," + "\"merkleRoot\":\"" + proof.merkleRoot() + "\","
				+ "\"signatureAlgorithm\":\"" + proof.signatureAlgorithm() + "\"," + "\"publicKey\":\""
				+ proof.publicKey() + "\"," + "\"signature\":\"" + proof.signature() + "\"" + "}";
	}
