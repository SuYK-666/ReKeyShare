...
public final class UserService {
	...
	public User registerPublicOnlyUser(String userId, UserRole role) {
		...
		User user = new User(userId, new com.example.pre.model.UserKeyPair(userId, transientPair.publicKey(), null))
				.withRole(role);
		users.save(user);
	...
	public User rotateUserKey(User current) {
		User rotated = new User(current.userId(), scheme.generateKeyPair(current.userId()), current.username(),
				current.role(), current.status(), current.createdAt());
		users.save(rotated);
		audit.record(new AuditEvent(Instant.now(), current.userId(), "USER_KEYPAIR_ROTATE", current.userId(), true,
				scheme.name()));
		return rotated;
	}
}
