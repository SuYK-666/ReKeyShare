...
public final class ProxyNodeService {
	...
	public ProxyNode register(String adminActor, String proxyId, String certificateFingerprint,
			Set<String> allowedTenantIds) {
		return register(adminActor, proxyId, certificateFingerprint, allowedTenantIds, Set.of(AlgorithmType.values()),
				Long.MAX_VALUE);
	}

	public ProxyNode register(String adminActor, String proxyId, String certificateFingerprint,
			Set<String> allowedTenantIds, Set<AlgorithmType> allowedSchemeIds, long quota) {
		ProxyNode node = ProxyNode.active(proxyId, certificateFingerprint, allowedTenantIds, allowedSchemeIds, quota);
		nodes.save(node);
		audit.record(new com.example.pre.model.AuditEvent(Instant.now(), adminActor, "PROXY_NODE_REGISTER", proxyId,
				true, certificateFingerprint));
		return node;
	}
	...
	public ProxyNode revoke(String adminActor, String proxyId) {
		...
		nodes.save(revoked);
		audit.record(new com.example.pre.model.AuditEvent(Instant.now(), adminActor, "PROXY_NODE_REVOKE", proxyId, true,
				"revoked"));
		return revoked;
	}
	...
	public synchronized void assertCanProxy(SecurityContext context, AlgorithmType scheme) {
		...
		if (context.credentialFingerprint() == null
				|| !node.certificateFingerprint().equals(context.credentialFingerprint())) {
			throw new ReKeyShareException(ErrorCode.ACCESS_DENIED, "proxy credential fingerprint mismatch");
		}
