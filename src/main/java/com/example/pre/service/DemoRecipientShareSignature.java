...
public final class DemoRecipientShareSignature {
	...
	public static String shareHash(RecipientReKeyShare share) {
		...
		return Hash.sha256Hex((share.sessionId() + "|" + share.challengeHash() + "|" + share.expiresAt() + "|"
				+ share.maskedScalarShare()).getBytes(StandardCharsets.UTF_8));
	}

	public static String message(ReKeySession session, String shareHash) {
		return session.sessionId() + "|" + session.challenge() + "|" + session.dataId() + "|" + session.ownerId() + "|"
				+ session.recipientId() + "|" + shareHash + "|" + session.expiresAt();
	}
