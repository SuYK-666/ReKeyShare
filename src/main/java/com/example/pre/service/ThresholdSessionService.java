...
public final class ThresholdSessionService {
    ...
    private final Set<String> consumedSessions = new HashSet<>();
    ...
    public ThresholdTranscript aggregate(ThresholdSession session, List<SignedThresholdShareV2> submissions,
                                         Instant now) {
        if (!session.activeAt(now) || consumedSessions.contains(session.sessionId())) {
        ...
        consumedSessions.add(session.sessionId());
    ...
    public boolean verify(ThresholdSession session, SignedThresholdShareV2 signed) {
        if (!signed.sessionId().equals(session.sessionId()) || !signed.grantId().equals(session.grantId())
                ...
                || !signed.contextHash().equals(contextHash(session))
                || !signed.shareDigest().equals(shareDigest(session, signed.proxyId(), signed.share(),
                signed.contextHash()))) {
            return false;
        }
    ...
    private static String contextHash(ThresholdSession session) {
        return Hash.sha256Hex(String.join("|", session.sessionId(), session.tenantId(), session.dataId(),
                session.grantId(), session.recipientId(), session.policyHash(),
                Integer.toString(session.contentKeyVersion()), session.capsuleHash(), session.proxyGroupId(),
                Long.toString(session.epoch())));
    }

    private static String shareDigest(ThresholdSession session, String proxyId, ThresholdReKeyShare share,
                                      String contextHash) {
        return Hash.sha256Hex(Bytes.concat(Bytes.utf8(String.join("|", "threshold-share-v2", proxyId,
                contextHash, Integer.toString(share.index()), Integer.toString(share.threshold()),
                Integer.toString(share.totalShares()))), share.value()));
    }
