...
public final class StateTransitionGuard {
	public void data(DataStatus from, DataStatus to) {
		require(from == DataStatus.ACTIVE
				&& (to == DataStatus.ROTATING || to == DataStatus.REVOKED || to == DataStatus.DELETED)
				|| from == DataStatus.ROTATING && (to == DataStatus.ACTIVE || to == DataStatus.REVOKED), "data", from,
				to);
	}

	public void grant(GrantStatus from, GrantStatus to) {
		require((from == GrantStatus.CREATED && to == GrantStatus.ACTIVE) || (from == GrantStatus.ACTIVE
				&& (to == GrantStatus.EXPIRED || to == GrantStatus.REVOKED || to == GrantStatus.ROTATED)), "grant",
				from, to);
	}

	public void dataPackage(PackageStatus from, PackageStatus to) {
		require(from == PackageStatus.ACTIVE
				&& (to == PackageStatus.EXPIRED || to == PackageStatus.INVALIDATED || to == PackageStatus.ROTATED),
				"package", from, to);
	}
