...
public final class ThresholdTranscriptVerifier {
	public boolean verify(ThresholdTranscript transcript) {
		return transcript.transcriptHash()
				.equals(hash(transcript.sessionId(), transcript.contextHash(), transcript.proxyIds(),
						transcript.shareDigests(), transcript.aggregateResultHash(), transcript.aggregatedAt()));
	}
