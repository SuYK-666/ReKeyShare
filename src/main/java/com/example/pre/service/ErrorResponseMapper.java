...
/**
 * Stable wire-level error classification. Internal service reasons remain available to audit.
 */
public final class ErrorResponseMapper {
    ...
    public int status(ErrorCode code) {
        return switch (code) {
            ...
            case ACCESS_DENIED, GRANT_REVOKED, GRANT_EXPIRED, GRANT_ROTATED, POLICY_VIOLATION,
                    CRYPTO_PROFILE_NOT_ALLOWED, DEMO_ONLY_API_DISABLED, PROXY_INACTIVE,
                    PROXY_QUOTA_EXCEEDED, SCHEME_NOT_ALLOWED, KEY_REVOKED, KEY_EXPIRED,
                    IDEMPOTENCY_CONFLICT -> 403;
    ...
    public String json(ErrorCode external, String requestId) {
        return "{\"success\":false,\"errorCode\":\"" + external.name() + "\",\"code\":\""
                + external.name() + "\",\"message\":\"" + message(external)
                + "\",\"traceId\":\"" + requestId + "\",\"requestId\":\"" + requestId
                + "\",\"eventId\":\"err-" + requestId + "\",\"timestamp\":\"" + Instant.now() + "\"}";
    }
}
