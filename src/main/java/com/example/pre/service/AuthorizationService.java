...
public final class AuthorizationService {
	...
	public ShareGrant createGrant(User owner, User recipient, EncryptedDataPackage dataPackage, AccessPolicy policy) {
		...
		ShareGrant grant = ShareGrant.active(dataPackage.dataId(), owner.userId(), recipient.userId(),
				dataPackage.algorithm(), policy, policyHash, createReKey(owner, recipient),
				dataPackage.contentKeyVersion(), dataPackage.tenantId());
		...
		audit.record(new AuditEvent(Instant.now(), owner.userId(), "GRANT_CREATE", grant.grantId(), true, policyHash)
				.withTenant(dataPackage.tenantId()));
		audit.record(
				new AuditEvent(Instant.now(), owner.userId(), "REKEY_GENERATE", grant.grantId(), true, scheme.name())
						.withTenant(dataPackage.tenantId()));
		return grant;
	}

	public ShareGrant createGrantWithRecipientShare(User owner, User recipient, EncryptedDataPackage dataPackage,
			AccessPolicy policy, RecipientReKeyShare share, ReKeySessionContext context) {
		ensureOwnerCanGrant(owner, dataPackage);
		if (dataPackage.algorithm() != AlgorithmType.ECC_PRE || scheme.algorithm() != AlgorithmType.ECC_PRE) {
			throw new ReKeyShareException(ErrorCode.ALGORITHM_MISMATCH,
					"ECC recipient share requires ECC data and ECC scheme");
		}
		if (!(owner.keyPair().privateKey() instanceof EccPrivateKeyMaterial ownerPrivate)
			...
			throw new ReKeyShareException(ErrorCode.INVALID_RECIPIENT_SHARE,
					"ECC recipient share requires ECC key material");
		}
		String policyHash = PolicyDigest.sha256(policy);
		ReEncryptionKey reKey = new EccInteractiveReKeyGenerator().generateReEncryptionKey(ownerPrivate,
				recipientPublic, share, context);
		ShareGrant grant = ShareGrant.active(dataPackage.dataId(), owner.userId(), recipient.userId(),
				dataPackage.algorithm(), policy, policyHash, reKey, dataPackage.contentKeyVersion(),
				dataPackage.tenantId());
		grant = bindReKeyScope(grant);
		saveGrant(grant);
		audit.record(new AuditEvent(Instant.now(), recipient.userId(), "RECIPIENT_SHARE_SUBMIT", grant.grantId(), true,
				"private-key-local").withTenant(dataPackage.tenantId()));
		audit.record(new AuditEvent(Instant.now(), owner.userId(), "GRANT_CREATE", grant.grantId(), true, policyHash)
				.withTenant(dataPackage.tenantId()));
		audit.record(
				new AuditEvent(Instant.now(), owner.userId(), "REKEY_GENERATE", grant.grantId(), true, scheme.name())
	...
	public ShareGrant createGrantFromVerifiedRecipientShare(User owner, User recipient,
			EncryptedDataPackage dataPackage, AccessPolicy policy, EccRecipientShareService shareService,
			String sessionId) {
		...
		if (!session.dataId().equals(dataPackage.dataId()) || !session.ownerId().equals(owner.userId())
		...
		ShareGrant grant = createGrantWithRecipientShare(owner, recipient, dataPackage, policy,
				submission.recipientShare(), session.cryptoContext());
	...
	public ReEncryptionKey createReKey(User owner, User recipient) {
		...
		if (owner.keyPair().privateKey() instanceof RsaPrivateKeyMaterial ownerPrivate
			...
			if (!(scheme instanceof RsaPreScheme rsaScheme)) {
				throw new ReKeyShareException(ErrorCode.ALGORITHM_MISMATCH,
						"RSA re-key requires RSA scheme parameters");
			}
	...
	private void ensureOwnerCanGrant(User owner, EncryptedDataPackage dataPackage) {
		if (!dataPackage.ownerId().equals(owner.userId())) {
			audit.record(new AuditEvent(Instant.now(), owner.userId(), "UNAUTHORIZED_ACCESS", dataPackage.dataId(),
					false, "ACCESS_DENIED"));
		...
		if (dataPackage.algorithm() != schemeFromUser(owner) || dataPackage.algorithm() != scheme.algorithm()) {
			throw new ReKeyShareException(ErrorCode.ALGORITHM_MISMATCH,
					"grant algorithm does not match service scheme");
		}
	}
}
