...
public final class RevocationService {
	...
	public RevocationService(PreScheme scheme, DataRepository dataRepository, GrantRepository grantRepository,
			ObjectAuthorizationService authorization, AuditRepository audit) {
	...
	public ShareGrant revokeGrant(String ownerId, String grantId, String reason) {
		return revokeGrant(new SecurityContext(ownerId, com.example.pre.model.UserRole.OWNER, "default", "legacy", 0,
				Long.MAX_VALUE), grantId, reason);
	}

	public ShareGrant revokeGrant(SecurityContext actor, String grantId, String reason) {
		ShareGrant existing = authorization.assertCanRevokeGrant(actor, grantId);
		new StateTransitionGuard().grant(existing.status(), GrantStatus.REVOKED);
		ShareGrant grant = existing.revoke(reason);
		grantRepository.save(grant);
		if (packageRepository != null) {
			packageRepository.findAll().stream().filter(dataPackage -> grantId.equals(dataPackage.grantId()))
					.filter(dataPackage -> dataPackage.status() == PackageStatus.ACTIVE).forEach(dataPackage -> {
		...
		audit.record(new AuditEvent(Instant.now(), actor.auditActor(), "GRANT_REVOKE", grantId, true,
				"packages invalidated; reason=" + reason).withTenant(actor.tenantId()));
		return grant;
	}
	...
	public EncryptedDataPackage acceptOwnerSideRotation(User owner, EncryptedDataPackage preparedRotation) {
		...
		if (preparedRotation.contentKeyVersion() != current.contentKeyVersion() + 1) {
			throw new ReKeyShareException(ErrorCode.AAD_MISMATCH,
					"owner-side rotation must increment contentKeyVersion by one");
		}
		...
		if (packageRepository != null) {
			packageRepository.findAll().stream().filter(dataPackage -> current.dataId().equals(dataPackage.dataId()))
					.filter(dataPackage -> dataPackage.status() == PackageStatus.ACTIVE).forEach(dataPackage -> {
						new StateTransitionGuard().dataPackage(dataPackage.status(), PackageStatus.ROTATED);
						packageRepository
								.save(dataPackage.invalidate(PackageStatus.ROTATED, "owner-side content key rotated"));
					});
		}
