...
public final class DataSecurityService {
	...
	public EncryptedDataPackage upload(User owner, byte[] plaintext) {
		return upload(
				new UploadDataCommand(owner, plaintext, "demo-" + SecureRandomUtil.randomId() + ".txt", "text/plain"));
	}
	...
	public EncryptedDataPackage upload(UploadDataCommand command) {
		...
		EncryptedDataPackage dataPackage = EncryptedDataPackage.uploadedEncrypted(dataId, owner.userId(),
				capsule.algorithm(), content.ciphertext(), content.nonce(), aad, capsule, plaintext.length,
				command.fileName(), command.contentType(), ownerKeyId, 1, policyHash,
	...
	public EncryptedDataPackage uploadEncrypted(UploadEncryptedCommand command) {
		...
		EncryptedDataPackage dataPackage = EncryptedDataPackage
				.uploadedEncrypted(command.context().dataId(), owner.userId(), command.context().algorithm(),
						command.encryptedContent(), command.contentNonce(), command.aad(), command.originalCapsule(),
						command.originalSize(), command.fileName(), command.contentType(),
						command.context().ownerKeyId(), command.context().contentKeyVersion(),
						command.context().policyHash(), com.example.pre.crypto.hash.Hash.sha256Hex(command.aad()))
				.withTenant(command.context().tenantId());
		dataRepository.save(dataPackage);
		audit.record(new AuditEvent(Instant.now(), owner.userId(), "UPLOAD_ENCRYPTED", dataPackage.dataId(), true,
				scheme.name()).withTenant(dataPackage.tenantId()));
		return dataPackage;
	}
	...
	public byte[] decryptOriginal(User owner, EncryptedDataPackage dataPackage) {
		byte[] dataKey = scheme.decapsulate(dataPackage.originalCapsule(), owner.keyPair().privateKey(),
				capsuleContext(dataPackage));
		try {
			byte[] plaintext = AesGcm.decrypt(dataKey, dataPackage.contentNonce(), dataPackage.encryptedContent(),
					dataPackage.aad());
			audit.record(new AuditEvent(Instant.now(), owner.userId(), "DECRYPT_ORIGINAL", dataPackage.dataId(), true,
					scheme.name()));
			return plaintext;
		} finally {
			Arrays.fill(dataKey, (byte) 0);
	...
	public byte[] decryptReEncrypted(User recipient, ReEncryptedPackage dataPackage) {
		...
		try {
			byte[] plaintext = AesGcm.decrypt(dataKey, dataPackage.contentNonce(), dataPackage.encryptedContent(),
					dataPackage.aad());
			audit.record(new AuditEvent(Instant.now(), recipient.userId(), "DECRYPT_REENCRYPTED", dataPackage.dataId(),
					true, scheme.name()));
			return plaintext;
		} finally {
			Arrays.fill(dataKey, (byte) 0);
	...
	public record UploadEncryptedCommand(User owner, byte[] encryptedContent, byte[] contentNonce, byte[] aad,
			EncryptedKeyCapsule originalCapsule, CapsuleContext context, long originalSize, String fileName,
			String contentType) {
	}
