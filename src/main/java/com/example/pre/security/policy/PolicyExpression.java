...
public record PolicyExpression(String tenantId, Set<String> allowedRoles, Set<String> allowedActions,
		String requiredPurpose, String requiredClassification, Instant notBefore, Instant expiresAt, int maxAccessCount,
	...
	public String canonical() {
		return String.join("|", tenantId, allowedRoles.stream().sorted().toList().toString(),
				allowedActions.stream().sorted().toList().toString(), requiredPurpose, requiredClassification,
				String.valueOf(notBefore), String.valueOf(expiresAt), Integer.toString(maxAccessCount),
				Boolean.toString(requireActiveProxy));
	}
}
