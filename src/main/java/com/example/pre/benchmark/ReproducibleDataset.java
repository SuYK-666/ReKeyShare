...
/**
 * Deterministic plaintext fixtures for performance experiments only. This
 * generator must never be used for cryptographic key or nonce material.
 */
public final class ReproducibleDataset {
	private static final long SEED = 0x52454b5348415245L;
	private static final List<String> DISTRIBUTIONS = List.of("deterministic-random", "zero-heavy", "text-json",
			"binary-image-like", "compressible");
	...
	public static byte[] generate(String distribution, int size, int round) {
		...
		switch (distribution) {
			...
			case "text-json" ->
				repeat(bytes, ("{\"tenant\":\"tenant-a\",\"event\":\"share\",\"round\":" + round + "}\n")
