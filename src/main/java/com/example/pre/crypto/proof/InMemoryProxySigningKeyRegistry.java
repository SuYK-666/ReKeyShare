...
public final class InMemoryProxySigningKeyRegistry implements ProxySigningKeyRegistry {
	...
	public synchronized ProxySigningKeyRecord activeForSigning(String proxyId, Instant at) {
		return records.values().stream()
				.filter(record -> record.proxyId().equals(proxyId) && record.usableForNewProof(at)).findFirst()
				.orElseGet(() -> rotate(proxyId, at));
	}
	...
	public synchronized ProxySigningKeyRecord rotate(String proxyId, Instant at) {
		records.replaceAll((id,
				current) -> current.proxyId().equals(proxyId) && current.status() == ProxySigningKeyRecord.Status.ACTIVE
						? new ProxySigningKeyRecord(current.proxyId(), current.keyId(), current.keyEpoch(),
								...
								ProxySigningKeyRecord.Status.RETIRED)
						: current);
	...
	public synchronized void revoke(String proxyId, String keyId) {
		...
		records.put(keyId,
				new ProxySigningKeyRecord(record.proxyId(), record.keyId(), record.keyEpoch(), record.publicKey(),
						record.privateKey(), record.notBefore(), record.notAfter(),
