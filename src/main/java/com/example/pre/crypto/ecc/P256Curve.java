...
public final class P256Curve {
	...
	public EccPoint add(EccPoint p, EccPoint q) {
		...
		BigInteger lambda = mod(q.y().subtract(p.y())).multiply(mod(q.x().subtract(p.x())).modInverse(P)).mod(P);
	...
	public boolean isOnCurve(EccPoint point) {
		...
		BigInteger right = point.x().multiply(point.x()).multiply(point.x()).add(A.multiply(point.x())).add(B).mod(P);
	...
	public byte[] encode(EccPoint point) {
		...
		return Bytes.concat(new byte[]{0x04}, Bytes.unsignedFixed(point.x(), FIELD_BYTES),
