...
public final class EccPreScheme implements PreScheme {
	...
	private EncryptedKeyCapsule encapsulateInternal(byte[] dataKey, PublicKeyMaterial ownerPublicKey,
		...
		try {
			...
			return new EncryptedKeyCapsule(AlgorithmType.ECC_PRE, curve.encode(c1), wrapped.ciphertext(),
					wrapped.nonce())
	...
	private EncryptedKeyCapsule reEncryptInternal(EncryptedKeyCapsule originalCapsule, ReEncryptionKey reKey) {
		...
		return new EncryptedKeyCapsule(AlgorithmType.ECC_PRE, curve.encode(transformed), originalCapsule.wrappedKey(),
