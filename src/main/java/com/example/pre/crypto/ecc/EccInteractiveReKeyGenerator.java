...
public final class EccInteractiveReKeyGenerator {
	public RecipientReKeyShare createRecipientShare(EccPrivateKeyMaterial recipientPrivateKey,
		...
		BigInteger maskedShare = recipientPrivateKey.scalar().modInverse(P256Curve.N).multiply(blind).mod(P256Curve.N);
		return new RecipientReKeyShare(maskedShare, context.sessionId(), challengeHash(context),
				context.createdAt().plusSeconds(900));
	}

	public ReEncryptionKey generateReEncryptionKey(EccPrivateKeyMaterial ownerPrivateKey,
			EccPublicKeyMaterial recipientPublicKey, RecipientReKeyShare recipientShare, ReKeySessionContext context) {
		...
		return new EccReEncryptionKey(ownerPrivateKey.scalar().multiply(unmaskedShare).mod(P256Curve.N));
	}
	...
	private static BigInteger contextBlind(ReKeySessionContext context) {
		BigInteger blind = Bytes.positiveBigInteger(
				Hash.sha256(("ReKeyShare-ECC-RecipientShare-v1|" + context.sessionId() + "|" + context.createdAt())
	...
	private static String challengeHash(ReKeySessionContext context) {
		return Hash.sha256Hex(("challenge|" + context.sessionId() + "|" + context.createdAt())
				.getBytes(java.nio.charset.StandardCharsets.UTF_8));
	}
}
