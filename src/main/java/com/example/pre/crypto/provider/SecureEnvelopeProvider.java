...
/**
 * Direct recipient envelope path based on JCA P-256 ECDH, HKDF-SHA256 and
 * AES-256-GCM. It deliberately does not claim proxy transformation semantics.
 */
public final class SecureEnvelopeProvider implements CryptoProvider {
		...
		try {
			...
			try {
				...
				return new EncryptedKeyCapsule(AlgorithmType.SECURE_ENVELOPE, ephemeral.getPublic().getEncoded(),
						wrapped.ciphertext(), wrapped.nonce()).bindContext(context == null ? "" : context.ownerKeyId(),
								context == null ? 1 : context.contentKeyVersion(), hash(aad), hash(aad),
	...
	public EncryptedKeyCapsule reEncrypt(EncryptedKeyCapsule capsule, ReEncryptionKey reKey, CapsuleContext context) {
		throw new UnsupportedOperationException(
				"secure envelope is direct-recipient wrapping, not proxy transformation");
	}
	...
	private static byte[] derive(PrivateKey privateKey, PublicKey publicKey, byte[] aad)
