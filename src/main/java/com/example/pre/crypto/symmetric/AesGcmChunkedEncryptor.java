...
public final class AesGcmChunkedEncryptor {
	...
	public record Manifest(int chunkSize, long totalPlaintextBytes, long totalCiphertextBytes,
	...
	public static Manifest encrypt(InputStream input, OutputStream output, byte[] key, byte[] baseAad)
			throws IOException {
		return encrypt(input, output, key, baseAad, DEFAULT_CHUNK_BYTES);
	}

	public static Manifest encrypt(InputStream input, OutputStream output, byte[] key, byte[] baseAad, int chunkSize)
			throws IOException {
	...
	public static String manifestHash(Manifest manifest) {
		...
		for (ChunkRecord chunk : manifest.chunks()) {
			sb.append('|').append(chunk.index()).append(':').append(HexFormat.of().formatHex(chunk.nonce())).append(':')
					.append(chunk.plaintextBytes()).append(':').append(chunk.ciphertextBytes()).append(':')
