...
public final class ScopedReEncryptionKey implements ReEncryptionKey {
	...
	public ReEncryptionKey consume(String actualGrantId, String actualDataId, String actualRecipientId,
		...
		if (!grantId.equals(actualGrantId) || !dataId.equals(actualDataId) || !recipientId.equals(actualRecipientId)
				|| ownerKeyVersion != actualKeyVersion || !policyHash.equals(actualPolicyHash)) {
	...
	/**
	 * Returns the scoped cryptographic material for durable storage adapters.
	 * Authorization counters remain persisted on the enclosing grant.
	 */
	public ReEncryptionKey snapshotDelegate() {
		return delegate;
	}
}
