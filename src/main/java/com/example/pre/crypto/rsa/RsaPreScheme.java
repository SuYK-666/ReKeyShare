...
public final class RsaPreScheme implements PreScheme {
	...
	public UserKeyPair generateKeyPair(String userId) {
		...
		return new UserKeyPair(userId, new RsaPublicKeyMaterial(parameters.modulus(), e),
	...
	private EncryptedKeyCapsule encapsulateInternal(byte[] dataKey, PublicKeyMaterial ownerPublicKey,
		...
		try {
			...
			return new EncryptedKeyCapsule(AlgorithmType.RSA_PRE, Bytes.unsignedFixed(c, parameters.modulusBytes()),
					wrapped.ciphertext(), wrapped.nonce())
	...
	private byte[] decapsulateInternal(EncryptedKeyCapsule capsule, PrivateKeyMaterial privateKey,
			CapsuleContext context) {
	...
	private EncryptedKeyCapsule reEncryptInternal(EncryptedKeyCapsule originalCapsule, ReEncryptionKey reKey) {
		...
		return new EncryptedKeyCapsule(AlgorithmType.RSA_PRE,
				Bytes.unsignedFixed(transformed, parameters.modulusBytes()), originalCapsule.wrappedKey(),
				originalCapsule.keyNonce()).bindContext(originalCapsule.ownerKeyId(), originalCapsule.ownerKeyVersion(),
						originalCapsule.aadHash(), originalCapsule.contextHash(), originalCapsule.parameterSpec());
	}
	...
	public ReEncryptionKey generateBaselineReEncryptionKey(RsaPrivateKeyMaterial ownerPrivateKey,
