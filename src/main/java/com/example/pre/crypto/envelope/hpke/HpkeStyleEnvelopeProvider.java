...
/**
 * Direct-recipient KEM/KDF/AEAD envelope. This provider is not PRE and exposes
 * no transform operation.
 */
public final class HpkeStyleEnvelopeProvider implements EnvelopeProvider {
	...
	public HpkeStyleSealedContentKey seal(HpkeStyleEnvelopeSealRequest request) {
		...
		try {
			...
			try {
				...
				return new HpkeStyleSealedContentKey(header, ephemeral.getPublic().getEncoded(), sealed.ciphertext(),
						sealed.nonce(), Hash.sha256Hex(aad));
			} finally {
	...
	public byte[] open(HpkeStyleEnvelopeOpenRequest request) {
		...
		try {
			PublicKey ephemeral = KeyFactory.getInstance("EC")
					.generatePublic(new X509EncodedKeySpec(request.sealedKey().encapsulatedPublicKey()));
	...
	public boolean verifyHeader(HpkeStyleSealedContentKey sealed, HpkeStyleEnvelopeOpenRequest expected) {
		...
		return SUITE.equals(h.envelopeVersion()) && SUITE.equals(h.algorithmSuite()) && "DHKEM-P256".equals(h.kemId())
				&& "HKDF-SHA256".equals(h.kdfId()) && "AES-256-GCM".equals(h.aeadId())
				&& h.tenantId().equals(expected.tenantId()) && h.dataId().equals(expected.dataId())
				&& h.recipientId().equals(expected.recipientId()) && h.policyHash().equals(expected.policyHash())
				&& h.contentKeyVersion() == expected.contentKeyVersion()
				&& Hash.sha256Hex(aad(new HpkeStyleEnvelopeHeader(h.envelopeVersion(), h.algorithmSuite(), h.kemId(),
						h.kdfId(), h.aeadId(), h.recipientKeyId(), h.tenantId(), h.dataId(), h.recipientId(),
						h.policyHash(), h.contentKeyVersion(), h.createdAt(), ""))).equals(h.aadHash())
	...
	private static byte[] aad(HpkeStyleEnvelopeHeader h) {
		return Bytes.utf8(String.join("|", h.envelopeVersion(), h.algorithmSuite(), h.kemId(), h.kdfId(), h.aeadId(),
				h.recipientKeyId(), h.tenantId(), h.dataId(), h.recipientId(), h.policyHash(),
