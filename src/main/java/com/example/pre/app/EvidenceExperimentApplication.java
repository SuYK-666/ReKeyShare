...
public final class EvidenceExperimentApplication {
	...
	public static void main(String[] args) throws Exception {
		String nonceRegistry = System.getProperty("rekeyshare.nonce.registry", "target/experiment/aes-gcm-nonces.txt");
	...
	private static void writeMetadata() throws IOException {
		String json = "{\"commit\":\"" + COMMIT + "\",\"jdk\":\"" + System.getProperty("java.version") + "\",\"os\":\""
	...
	private static void runSecureEnvelopeCorrectness() throws IOException {
		...
		for (int size : sizes) {
			for (int round = 1; round <= roundsPerSize; round++) {
				...
				rows.add(String.format(Locale.ROOT, "%d,%d,%.4f,%.4f,%.4f,%.4f,%s,%s", size, round, encryption,
						encapsulate, decapsulate, decryption, success, rejected));
			}
		}
		...
		Files.writeString(SUMMARY.resolve("e01-correctness-summary.md"), header("E01 End-to-End Correctness")
				+ "Secure envelope AEAD rounds over 1 KB, 100 KB, 1 MB and 10 MB files: " + rounds + "; recovered: "
				+ recovered + "; unauthorized recipient rejected: " + unauthorizedRejected + ".\n\n"
				...
				+ (pass ? "PASS" : "FAIL") + "** (success and unauthorized rejection must both be 100%).\n");
	}
	...
	private static void runDatasetDistribution() throws IOException {
		...
		for (String distribution : ReproducibleDataset.distributions()) {
			...
			summary.append(String.format(Locale.ROOT, "| %s | %d | %.4f | %.4f | %s |\n", distribution, rounds,
					encryptionTotal / rounds, decryptionTotal / rounds, allSuccess ? "PASS" : "FAIL"));
		}
	...
	private static void runChunkedAead() throws IOException {
		...
		for (int sizeIndex = 0; sizeIndex < sizes.length; sizeIndex++) {
			...
			for (int round = 1; round <= roundsPerSize; round++) {
				...
				try (InputStream in = new PatternInputStream(size);
						OutputStream out = Files.newOutputStream(encrypted)) {
				...
				rows.add(String.format(Locale.ROOT, "%d,%d,%d,%d,%.4f,%.4f,%d,%.4f,%d,true,%s", size, round, chunkSize,
						manifest.chunks().size(), encryption, decryption, memory.deltaBytes(), heapPercent, chunkSize,
						success));
		...
		Files.writeString(RAW.resolve("e03-chunk-integrity-results.json"),
				"{\"cases\":4,\"detected\":" + integrityRejected
						+ ",\"mutations\":[\"modify\",\"delete\",\"reorder\",\"aad-replace\"]," + "\"result\":\""
						+ (integrityRejected == 4 ? "PASS" : "FAIL") + "\"}");
		Files.writeString(SUMMARY.resolve("e03-chunked-aead-summary.md"), header("E03 Chunked AEAD Scalability")
				...
				+ String.format(Locale.ROOT,
						...
						maxHeapPercent[0], maxHeapPercent[1], maxHeapPercent[2], hundredMbMemoryPass ? "PASS" : "FAIL")
	...
	private static int chunkIntegrityCases(byte[] key, byte[] aad, int chunkSize) throws IOException {
		...
		AesGcmChunkedEncryptor.Manifest manifest = AesGcmChunkedEncryptor
				.encrypt(new PatternInputStream(2 * integrityChunkSize), encrypted, key, aad, integrityChunkSize);
	...
	private static void runPackageTamper() throws IOException {
		...
		String json = "{\"cases\":5,\"detected\":" + detected + ",\"detectionRate\":" + (detected * 20)
				+ ",\"result\":\"" + (detected == fields.length ? "PASS" : "FAIL") + "\"}";
		Files.writeString(RAW.resolve("e04-package-tamper-results.json"), json);
		Files.writeString(SUMMARY.resolve("e04-package-tamper-summary.md"),
				header("E04 Package Tamper Detection")
						+ "Mutations covered ciphertext, AAD, capsule, policy binding and manifest hash.\n\n"
						+ "Raw data: `../raw/e04-package-tamper-results.json`\n\n" + "Result: **"
						+ (detected == fields.length ? "PASS" : "FAIL") + "** (" + detected + "/5 detected).\n");
	}
	...
	private static void runThreshold() throws IOException {
		...
		boolean pass = thresholdCase(rows, 2, 3, 1, 0, false) & thresholdCase(rows, 3, 5, 2, 0, false);
		...
		Files.writeString(SUMMARY.resolve("e13-threshold-summary.md"),
				header("E13 Threshold Prototype")
	...
	private static void runNegativePolicyMatrix() throws IOException {
		...
		PolicyExpression policy = new PolicyExpression("tenant-a", Set.of("RECIPIENT"), Set.of("download"), "research",
				"confidential", now.minusSeconds(60), now.plusSeconds(60), 3, true);
		...
		for (int index = 0; index < 50; index++) {
			PolicyRequest request = switch (index % 10) {
				...
				case 8 -> request("RECIPIENT", "tenant-a", "download", "research", "confidential", 0, true,
						now.minusSeconds(120));
				default -> request("RECIPIENT", "tenant-a", "download", "research", "confidential", 0, true,
						now.plusSeconds(120));
			};
			...
			json.append("{\"id\":").append(index + 1).append(",\"decision\":\"")
					...
					.append(decision.reasonCode()).append("\",\"policyHash\":\"").append(decision.policyHash())
					.append("\"}");
		}
		json.append("],\"total\":50,\"rejected\":").append(rejected).append(",\"serverErrors\":0,\"result\":\"")
				.append(rejected == 50 ? "PASS" : "FAIL").append("\"}");
		...
		Files.writeString(SUMMARY.resolve("e05-negative-policy-summary.md"), header("E05 Negative Authorization Matrix")
				...
				+ (rejected == 50 ? "PASS" : "FAIL") + "** (" + rejected + "/50 rejected; server error count 0).\n");
	}
	...
	private static PolicyRequest request(String role, String tenant, String action, String purpose,
		...
		return new PolicyRequest("recipient", role, tenant, "data", classification, action, purpose, count, proxyActive,
				now);
	}
	...
	private static boolean thresholdCase(List<String> rows, int threshold, int total, int submitted, int round,
		...
		rows.add(String.format(Locale.ROOT, "%d,%d,%d,%d,%s,%s,%s,%.4f", threshold, total, submitted, round, expected,
				actual, proofsVerified, elapsed(start)));
	...
	private static SharedPackageV2 tamper(SharedPackageV2 issued, String field) {
		...
		ReEncryptedPackage altered = switch (field) {
			case "ciphertext" ->
				replace(p, Bytes.utf8("other-ciphertext"), p.aad(), p.reEncryptedCapsule(), p.grantPolicyHash());
			case "aad" ->
				replace(p, p.encryptedContent(), Bytes.utf8("other-aad"), p.reEncryptedCapsule(), p.grantPolicyHash());
			case "capsule" -> replace(p, p.encryptedContent(), p.aad(), new EncryptedKeyCapsule(AlgorithmType.RSA_PRE,
					Bytes.utf8("changed"), Bytes.utf8("wrapped"), Bytes.utf8("nonce")), p.grantPolicyHash());
	...
	private static ReEncryptedPackage packageFixture(byte[] ciphertext) {
		return new ReEncryptedPackage("data-1", "alice", "bob", AlgorithmType.RSA_PRE, ciphertext,
				Bytes.utf8("content-nonce"), Bytes.utf8("aad"), new EncryptedKeyCapsule(AlgorithmType.RSA_PRE,
						Bytes.utf8("header"), Bytes.utf8("wrapped"), Bytes.utf8("nonce")),
				Instant.now());
	}
