...
import com.example.pre.storage.JdbcDataRepository;
import com.example.pre.storage.JdbcGrantRepository;
...
import com.example.pre.storage.JdbcReEncryptedPackageRepository;
...
public final class ReKeyShareApplication {
	...
	public static RunningServer start(int requestedPort, RuntimeProfile profile) throws IOException {
		...
		server.setExecutor(
				new ThreadPoolExecutor(4, 32, 30, TimeUnit.SECONDS, new ArrayBlockingQueue<>(128), runnable -> {
	...
	private static void route(HttpExchange exchange, ApiState state) throws IOException {
		...
		try {
			...
			DemoTokenService.AuthenticatedActor auth = publicEndpoint(method, path)
					? new DemoTokenService.AuthenticatedActor("anonymous", UserRole.RECIPIENT, "public", "public", 0,
							Long.MAX_VALUE)
			...
			if ("GET".equals(method) && "/".equals(path)) {
				writeJson(exchange, 200,
						"{\"name\":\"ReKeyShare\",\"status\":\"running\",\"openapi\":\"/openapi.json\"}");
				...
				getData(exchange, state, security, segment(path, 3));
			} else if ("POST".equals(method) && "/api/grants".equals(path)) {
				createGrant(exchange, state, security, body);
			} else if ("POST".equals(method) && "/api/grants/ecc".equals(path)) {
				createEccGrant(exchange, state, security, body);
			} else if ("GET".equals(method) && "/api/grants".equals(path)) {
				writeJson(exchange, 200, "{\"count\":" + state.grants.findByTenant(security.tenantId()).size() + "}");
			} else if ("POST".equals(method) && path.matches("/api/grants/[^/]+/revoke")) {
				revokeGrant(exchange, state, security, segment(path, 3));
			} else if ("POST".equals(method) && "/api/rekey-sessions".equals(path)) {
				createReKeySession(exchange, state, security, body);
				...
				getSharedPackage(exchange, state, security, segment(path, 3));
			} else if (state.profile.demoFeaturesEnabled() && "GET".equals(method)
					&& path.matches("/api/demo/shared-packages/[^/]+/decrypt")) {
				demoDecryptSharedPackage(exchange, state, security, segment(path, 4));
			} else if ("GET".equals(method) && "/api/audit/events".equals(path)) {
				requireAuditor(auth);
				auditEvents(exchange, state, security);
			} else if ("GET".equals(method) && path.matches("/api/audit/data/[^/]+")) {
				requireAuditor(auth);
				String dataId = segment(path, 4);
				long count = state.auditRepository.findByTenant(security.tenantId()).stream()
						.filter(event -> dataId.equals(event.target()) || dataId.equals(event.dataId())).count();
				writeJson(exchange, 200,
						"{\"code\":\"SUCCESS\",\"dataId\":\"" + json(dataId) + "\",\"count\":" + count + "}");
				...
				writeJson(exchange, 200, "{\"rootHash\":\"" + result.rootHash() + "\",\"checkedEvents\":"
				...
				requireAdmin(auth);
				if (state.auditRepository instanceof InMemoryAuditRepository memory && memory.findAll().size() > 1) {
				...
				state.audit.record("benchmark", "BENCHMARK_RUN", "docs/reports/raw/e02-algorithm-benchmark.csv", true,
						"api");
				writeJson(exchange, 201,
						"{\"code\":\"SUCCESS\",\"result\":\"docs/reports/raw/e02-algorithm-benchmark.csv\"}");
				...
				requireAuditor(auth);
				writeJson(exchange, 200,
						state.benchmark.summaryJson(Path.of("docs", "reports", "raw", "e02-algorithm-benchmark.csv")));
				...
				writeJson(exchange, 200,
						"{\"code\":\"SUCCESS\",\"path\":\"" + json(input.toString()) + "\",\"valid\":" + valid + "}");
				...
				writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"mode\":\""
						+ (state.profile.durableLocalSecurityStores()
								? "secure-local:h2-audit+replay+idempotency+proxy+data+grant+package"
								: "memory+json-snapshot")
						+ "\",\"users\":" + state.users.findAll().size() + ",\"dataObjects\":"
						+ state.dataRepository.findAll().size() + ",\"grants\":" + state.grants.findAll().size()
						+ ",\"packages\":" + state.packages.findAll().size() + ",\"objectStore\":\""
						+ state.objectStore.getClass().getSimpleName() + "\",\"keyProvider\":\""
						+ (state.localKeyProvider == null
								? "external-boundary"
								: state.localKeyProvider.getClass().getSimpleName())
						+ "\"}");
			} else {
	...
	private static void createUser(HttpExchange exchange, ApiState state, Map<String, String> body) throws IOException {
		String userId = body.getOrDefault("userId",
				body.getOrDefault("username", "user-" + state.users.findAll().size()));
		...
		writeJson(exchange, 201,
				"{\"code\":\"SUCCESS\",\"userId\":\"" + json(user.userId()) + "\",\"role\":\"" + user.role()
						+ "\",\"algorithm\":\"" + algorithm + "\",\"algorithmSuite\":\"" + descriptor.schemeId()
						+ "\",\"securityLevel\":\"" + descriptor.securityLevel() + "\",\"securityNotice\":\""
						+ descriptor.proofStatus() + "\",\"token\":\"" + token + "\"}");
	}
	...
	private static void login(HttpExchange exchange, ApiState state, Map<String, String> body) throws IOException {
		...
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"userId\":\"" + json(user.userId()) + "\",\"role\":\""
				+ user.role() + "\",\"token\":\"" + state.tokens.issue(user) + "\"}");
	}
	...
	private static void createKey(HttpExchange exchange, ApiState state, DemoTokenService.AuthenticatedActor actor,
		...
		writeJson(exchange, 201,
				"{\"code\":\"SUCCESS\",\"keyId\":\"" + key.keyId() + "\",\"version\":" + key.version() + "}");
	}

	private static void rotateUserKey(HttpExchange exchange, ApiState state, String actor, String userId)
			throws IOException {
		...
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"userId\":\"" + json(rotated.userId()) + "\",\"keyId\":\""
				+ key.keyId() + "\",\"version\":" + key.version() + ",\"fingerprint\":\"" + key.fingerprint() + "\"}");
	}

	private static void uploadData(HttpExchange exchange, ApiState state, String actor, Map<String, String> body)
			throws IOException {
		...
		EncryptedDataPackage data = state.data(requested).upload(new DataSecurityService.UploadDataCommand(owner,
				plaintextBytes, fileName, body.getOrDefault("contentType", "text/plain")));
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"dataId\":\"" + data.dataId() + "\",\"contentKeyVersion\":"
	...
	private static void uploadEncryptedData(HttpExchange exchange, ApiState state, SecurityContext security,
		...
		byte[] aad = body.containsKey("aad")
				...
				: com.example.pre.util.AadBuilder.build(context);
		EncryptedKeyCapsule capsule = new EncryptedKeyCapsule(requested, b64decode(body.get("capsuleHeader")),
				b64decode(body.get("wrappedKey")), b64decode(body.get("keyNonce"))).bindContext(ownerKeyId,
						contentKeyVersion,
		...
		EncryptedDataPackage data = state.data(requested)
				.uploadEncrypted(new DataSecurityService.UploadEncryptedCommand(owner,
						b64decode(body.get("encryptedContent")), b64decode(body.get("contentNonce")), aad, capsule,
						context, Long.parseLong(body.getOrDefault("originalSize", "-1")),
		...
		writeJson(exchange, 201,
				"{\"code\":\"SUCCESS\",\"dataId\":\"" + data.dataId() + "\",\"contentKeyVersion\":"
						+ data.contentKeyVersion() + ",\"ciphertextHash\":\"" + data.ciphertextHash()
						+ "\",\"algorithmSuite\":\"" + descriptor.schemeId() + "\",\"securityLevel\":\""
						+ descriptor.securityLevel() + "\",\"securityNotice\":\"" + descriptor.proofStatus() + "\"}");
	}

	private static void getData(HttpExchange exchange, ApiState state, SecurityContext actor, String dataId)
			throws IOException {
		state.objectAuth.assertCanReadData(actor, dataId);
		EncryptedDataPackage data = state.dataRepository.findByTenantAndId(actor.tenantId(), dataId)
				.orElseThrow(() -> new ReKeyShareException(ErrorCode.DATA_NOT_FOUND, "data not found"));
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"dataId\":\"" + data.dataId() + "\",\"ownerId\":\""
				+ data.ownerId() + "\",\"ciphertextHash\":\"" + data.ciphertextHash() + "\"}");
	}

	private static void createGrant(HttpExchange exchange, ApiState state, SecurityContext actor,
			Map<String, String> body) throws IOException {
		...
		User owner = requireUser(state, actor.userId());
		...
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"grantId\":\"" + grant.grantId() + "\",\"policyHash\":\""
				+ grant.policyHash() + "\"}");
	}

	private static void createEccGrant(HttpExchange exchange, ApiState state, SecurityContext actor,
			Map<String, String> body) throws IOException {
		...
		EncryptedDataPackage data = state.dataRepository.findByTenantAndId(actor.tenantId(), dataId)
				.orElseThrow(() -> new ReKeyShareException(ErrorCode.DATA_NOT_FOUND, "data not found"));
		AccessPolicy policy = policyFromBody(body);
		ShareGrant grant = state.authorization(AlgorithmType.ECC_PRE).createGrantFromVerifiedRecipientShare(owner,
				recipient, data, policy, state.eccShares, sessionId);
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"grantId\":\"" + grant.grantId() + "\",\"sessionId\":\""
				+ json(sessionId) + "\",\"policyHash\":\"" + grant.policyHash() + "\"}");
	}
	...
	private static AccessPolicy policyFromBody(Map<String, String> body) {
		...
		int maxReEncryptCount = Integer
				.parseInt(body.getOrDefault("maxReEncryptCount", String.valueOf(maxAccessCount)));
		...
		return new AccessPolicy(Boolean.parseBoolean(body.getOrDefault("allowPreview", "true")),
				Boolean.parseBoolean(body.getOrDefault("allowDownload", "true")),
				Boolean.parseBoolean(body.getOrDefault("allowReshare", "false")), maxAccessCount, maxReEncryptCount,
				maxDownloadCount, maxDecryptCount, Instant.now().plus(expiresInSeconds, ChronoUnit.SECONDS),
				body.getOrDefault("purpose", "api-demo"), body.getOrDefault("allowedActions", "download,decrypt"));
	}

	private static void createReKeySession(HttpExchange exchange, ApiState state, SecurityContext actor,
			Map<String, String> body) throws IOException {
		requireDemoFeature(state, "ECC baseline rekey session");
		String dataId = body.get("dataId");
		String recipientId = body.get("recipientId");
		state.objectAuth.assertCanCreateGrant(actor, dataId);
		ReKeySession session = state.eccShares.createSession(dataId, actor.userId(), recipientId);
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"sessionId\":\"" + session.sessionId() + "\",\"challenge\":\""
				+ session.challenge() + "\",\"expiresAt\":\"" + session.expiresAt() + "\"}");
	}

	private static void submitRecipientShare(HttpExchange exchange, ApiState state, String actor, String sessionId,
			Map<String, String> body) throws IOException {
		...
		RecipientShareSubmission submission = new RecipientShareSubmission(sessionId, session.dataId(),
				session.ownerId(), actor, body.getOrDefault("challenge", session.challenge()), share,
				body.getOrDefault("recipientShareHash", DemoRecipientShareSignature.shareHash(share)),
				body.get("signature"), Instant.now());
		state.eccShares.submitRecipientShare(submission);
		writeJson(exchange, 201,
				"{\"code\":\"SUCCESS\",\"sessionId\":\"" + sessionId + "\",\"status\":\"SHARE_SUBMITTED\"}");
	}

	private static void submitRecipientShareDemo(HttpExchange exchange, ApiState state, String actor, String sessionId)
			throws IOException {
		ReKeySession session = state.eccShares.requireSession(sessionId);
		if (!session.recipientId().equals(actor)) {
			throw new ReKeyShareException(ErrorCode.ACCESS_DENIED, "only session recipient can submit share");
		...
		RecipientReKeyShare share = DemoPrivateKeyStore.createEccRecipientShareLocally(recipient,
				session.cryptoContext());
		RecipientShareSubmission submission = new RecipientShareSubmission(sessionId, session.dataId(),
				session.ownerId(), actor, session.challenge(), share, DemoRecipientShareSignature.shareHash(share),
				DemoRecipientShareSignature.sign(session, share), Instant.now());
		state.eccShares.submitRecipientShare(submission);
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"sessionId\":\"" + sessionId
				+ "\",\"status\":\"SHARE_SUBMITTED\",\"shareHash\":\"" + submission.recipientShareHash() + "\"}");
	}

	private static void revokeGrant(HttpExchange exchange, ApiState state, SecurityContext actor, String grantId)
			throws IOException {
		ShareGrant existing = state.grants.findByTenantAndId(actor.tenantId(), grantId)
				.orElseThrow(() -> new ReKeyShareException(ErrorCode.GRANT_NOT_FOUND, "grant not found"));
		ShareGrant grant = state.revocation(existing.algorithm()).revokeGrant(actor, grantId, "owner requested revoke");
		writeJson(exchange, 200,
				"{\"code\":\"SUCCESS\",\"grantId\":\"" + grant.grantId() + "\",\"status\":\"" + grant.status() + "\"}");
	}

	private static void proxyReEncrypt(HttpExchange exchange, ApiState state, SecurityContext actor,
			Map<String, String> body) throws IOException {
		requireDemoFeature(state, "baseline proxy transformation");
		ShareGrant grant = state.grants.findByTenantAndId(actor.tenantId(), body.get("grantId"))
				.orElseThrow(() -> new ReKeyShareException(ErrorCode.GRANT_NOT_FOUND, "grant not found"));
		SecurityContext boundProxy = actor
				.withCredentialFingerprint(body.getOrDefault("certificateFingerprint", "default-service-token"));
		ReEncryptedPackage dataPackage = state.proxy(grant.algorithm()).reEncrypt(boundProxy, body.get("grantId"));
		writeJson(exchange, 201, "{\"code\":\"SUCCESS\",\"packageId\":\"" + dataPackage.packageId()
				+ "\",\"grantId\":\"" + dataPackage.grantId() + "\"}");
	}

	private static void registerProxyNode(HttpExchange exchange, ApiState state, String actor, Map<String, String> body)
			throws IOException {
		...
		Set<AlgorithmType> allowedSchemes = java.util.Arrays
				.stream(body.getOrDefault("allowedSchemeIds", "RSA_PRE,ECC_PRE,SECURE_ENVELOPE").split(","))
				.map(String::trim).map(String::toUpperCase).map(AlgorithmType::valueOf)
	...
	private static void revokeProxyNode(HttpExchange exchange, ApiState state, String actor, String proxyId)
			throws IOException {
		state.proxyNodes.revoke(actor, proxyId);
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"proxyId\":\"" + json(proxyId) + "\",\"status\":\"REVOKED\"}");
	}

	private static void getSharedPackage(HttpExchange exchange, ApiState state, SecurityContext actor, String packageId)
			throws IOException {
		...
		synchronized (state.grants) {
			...
			grant = state.grants.findByTenantAndId(actor.tenantId(), dataPackage.grantId())
					.orElseThrow(() -> new ReKeyShareException(ErrorCode.GRANT_NOT_FOUND, "grant not found"));
			...
			try {
				...
				state.audit.record(actor.auditActor(), "CONVERSION_PROOF_VERIFY_FAILED", packageId, false,
						e.code().name());
				throw e;
			}
			state.grants.save(grant.incrementDownload());
		}
		state.audit.record(actor.auditActor(), "DOWNLOAD_PACKAGE", packageId, true, dataPackage.grantId());
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"packageVersion\":\"" + v2.packageVersion()
				+ "\",\"schemeId\":\"" + v2.schemeId() + "\",\"algorithmSuite\":\"" + v2.schemeId()
				+ "\",\"securityLevel\":\"" + descriptorFor(dataPackage.algorithm()).securityLevel()
				+ "\",\"securityNotice\":\"" + v2.proofStatus() + "\",\"parameterSpec\":\"" + json(v2.parameterSpec())
				+ "\",\"proofStatus\":\"" + v2.proofStatus() + "\",\"conversionProofDigest\":\""
				+ com.example.pre.service.ConversionProofService.digest(dataPackage.conversionProof())
				+ "\",\"conversionProofProxyId\":\"" + json(dataPackage.conversionProof().proxyId())
				+ "\",\"packageId\":\"" + packageId + "\",\"dataId\":\"" + dataPackage.dataId() + "\",\"ownerId\":\""
				+ dataPackage.ownerId() + "\",\"recipientId\":\"" + dataPackage.recipientId() + "\",\"algorithm\":\""
				+ dataPackage.algorithm() + "\",\"contentKeyVersion\":" + dataPackage.contentKeyVersion()
				+ ",\"ciphertext\":\"" + b64(dataPackage.encryptedContent()) + "\",\"nonce\":\""
				+ b64(dataPackage.contentNonce()) + "\",\"aad\":\"" + b64(dataPackage.aad()) + "\",\"grantAad\":\""
				+ b64(dataPackage.grantAad()) + "\",\"reEncryptedCapsule\":{\"capsuleId\":\""
				+ dataPackage.reEncryptedCapsule().capsuleId() + "\",\"algorithm\":\""
				+ dataPackage.reEncryptedCapsule().algorithm() + "\",\"parameterSpec\":\""
				+ dataPackage.reEncryptedCapsule().parameterSpec() + "\",\"header\":\""
				+ b64(dataPackage.reEncryptedCapsule().header()) + "\",\"wrappedKey\":\""
				+ b64(dataPackage.reEncryptedCapsule().wrappedKey()) + "\",\"keyNonce\":\""
				+ b64(dataPackage.reEncryptedCapsule().keyNonce()) + "\",\"aadHash\":\""
				+ dataPackage.reEncryptedCapsule().aadHash() + "\",\"contextHash\":\""
				+ dataPackage.reEncryptedCapsule().contextHash() + "\"}" + ",\"ciphertextStoragePath\":\""
				+ json(dataPackage.ciphertextStoragePath()) + "\",\"policyHash\":\"" + dataPackage.policyHash()
				+ "\",\"grantPolicyHash\":\"" + dataPackage.grantPolicyHash() + "\",\"grantContextHash\":\""
				+ dataPackage.grantContextHash() + "\",\"manifestHash\":\"" + v2.manifest().manifestHash()
				+ "\",\"manifestFormatVersion\":\"" + v2.manifest().formatVersion() + "\",\"minVerifierVersion\":\""
				+ v2.manifest().minVerifierVersion() + "\",\"ciphertextHash\":\"" + v2.manifest().ciphertextHash()
				+ "\",\"aadHash\":\"" + v2.manifest().aadHash() + "\",\"capsuleHash\":\"" + v2.manifest().capsuleHash()
				+ "\",\"status\":\"" + dataPackage.status() + "\"}");
	}

	private static void demoDecryptSharedPackage(HttpExchange exchange, ApiState state, SecurityContext actor,
		...
		ShareGrant grant = state.grants.findByTenantAndId(actor.tenantId(), dataPackage.grantId())
				.orElseThrow(() -> new ReKeyShareException(ErrorCode.GRANT_NOT_FOUND, "grant not found"));
		User recipient = requireUser(state, actor.userId());
		...
		state.audit.record(actor.auditActor(), "DEMO_DECRYPT_SHARED", packageId, true, dataPackage.grantId());
		writeJson(exchange, 200, "{\"code\":\"SUCCESS\",\"packageId\":\"" + packageId + "\",\"plaintext\":\""
				+ json(new String(plaintext, StandardCharsets.UTF_8)) + "\"}");
	}

	private static void auditEvents(HttpExchange exchange, ApiState state, SecurityContext actor) throws IOException {
		java.util.List<AuditEvent> events = state.auditRepository.findByTenant(actor.tenantId());
		StringBuilder sb = new StringBuilder();
		sb.append("{\"count\":").append(events.size()).append(",\"events\":[");
	...
	private static void auditVerify(HttpExchange exchange, ApiState state) throws IOException {
		...
		state.audit.record("audit", "AUDIT_VERIFY", "audit-chain", before.valid(),
				"checkedEvents=" + before.checkedEvents());
		AuditService.AuditVerificationResult after = state.audit.verifyChain();
		writeJson(exchange, 200,
				"{" + "\"valid\":" + before.valid() + "," + "\"checkedEvents\":" + before.checkedEvents() + ","
						+ "\"brokenAt\":" + (before.brokenAt() == null ? "null" : before.brokenAt()) + ","
						+ "\"rootBeforeVerify\":\"" + before.rootHash() + "\"," + "\"rootAfterVerify\":\""
						+ after.rootHash() + "\"" + "}");
	}
	...
	private static DemoTokenService.AuthenticatedActor actor(HttpExchange exchange, Map<String, String> body,
			ApiState state) {
		...
		if (state.legacyActorHeaderEnabled) {
			...
			if (header != null && !header.isBlank()) {
				...
				return new DemoTokenService.AuthenticatedActor(header, user == null ? defaultRole(header) : user.role(),
						"legacy", "legacy", now, now + 300);
			}
		}
		...
		if (!fallback.isBlank() && state.legacyActorHeaderEnabled) {
			...
			long now = Instant.now().getEpochSecond();
			return new DemoTokenService.AuthenticatedActor(fallback, user == null ? defaultRole(fallback) : user.role(),
					"legacy", "legacy", now, now + 300);
		}
	...
	private static boolean publicEndpoint(String method, String path) {
		return ("GET".equals(method) && "/".equals(path)) || ("GET".equals(method) && "/openapi.json".equals(path))
	...
	private static void requireDemoFeature(ApiState state, String feature) {
		if (!state.profile.demoFeaturesEnabled()) {
			throw new ReKeyShareException(ErrorCode.DEMO_ONLY_API_DISABLED,
					"demo feature is disabled in production profile: " + feature);
		}
	}
	...
	private static void enforceContentType(HttpExchange exchange, String method) {
		...
		if (!normalized.startsWith("application/json") && !normalized.startsWith("application/x-www-form-urlencoded")) {
	...
	private static int httpStatus(ErrorCode code) {
		return switch (code) {
			...
			case INVALID_REQUEST, ALGORITHM_MISMATCH, CAPSULE_TAMPERED, CIPHERTEXT_TAMPERED, PACKAGE_INVALID,
					INVALID_RECIPIENT_SHARE, INVALID_REKEY_SESSION, CRYPTO_CONTEXT_MISMATCH ->
				400;
			default -> 403;
		};
	}
	...
	private static String errorJson(String code, String message, String requestId) {
		...
		return "{\"success\":false,\"errorCode\":\"" + json(code) + "\",\"code\":\"" + json(code) + "\",\"message\":\""
				+ json(message) + "\",\"traceId\":\"" + json(requestId) + "\",\"requestId\":\"" + json(requestId)
				+ "\",\"eventId\":\"" + eventId + "\",\"timestamp\":\"" + Instant.now() + "\"}";
	}
	...
	private static SchemeDescriptor descriptorFor(AlgorithmType algorithm) {
		return switch (algorithm) {
			case RSA_PRE -> new SchemeDescriptor("RSA_PRE_BASELINE", "RSA common-modulus transformation",
					"EXPERIMENTAL", "RSA-PRE-baseline", true, true, false, "NOT_PRODUCTION_REVIEWED", "IMPLEMENTED");
			case ECC_PRE -> new SchemeDescriptor("ECC_PRE_BASELINE", "Experimental P-256 transformation",
					"EXPERIMENTAL", "ECC-PRE-P-256-demo", true, true, false, "NOT_PRODUCTION_REVIEWED", "IMPLEMENTED");
			case SECURE_ENVELOPE -> new SchemeDescriptor("SECURE_ENVELOPE_V1", "ECDH-KEM envelope", "128-bit",
					"P-256/HKDF-SHA256/AES-256-GCM", false, false, false, "JCA_PRIMITIVES_SECURITY_BOUNDARY_DOCUMENTED",
					"IMPLEMENTED");
		};
	}
	...
	private static String openApiJson(RuntimeProfile profile) {
		StringBuilder sb = new StringBuilder();
		sb.append("{\"openapi\":\"3.0.3\",").append("\"info\":{\"title\":\"ReKeyShare API\",\"version\":\"1.0.0\"},")
				.append("\"security\":[{\"bearerAuth\":[]}],").append("\"paths\":{");
		...
		appendPath(sb, "/api/data/upload-encrypted", "post", true,
				"Upload client-side encrypted content and PRE capsule");
		...
		if (profile.demoFeaturesEnabled()) {
			...
			appendPath(sb, "/api/rekey-sessions/{sessionId}/recipient-share", "post", true,
					"Demo signed ECC recipient share");
		...
		if (profile.demoFeaturesEnabled()) {
			appendPath(sb, "/api/demo/shared-packages/{packageId}/decrypt", "get", true,
					"Demo-only plaintext verification");
		}
	...
	private static void appendPath(StringBuilder sb, String path, String method, boolean secured, String summary) {
		sb.append('"').append(path).append("\":{\"").append(method).append("\":{\"summary\":\"").append(summary)
	...
	private static final class ApiState {
		...
		final ReEncryptedPackageRepository packages;
		...
		ApiState(RuntimeProfile profile) {
			...
			if (profile.durableLocalSecurityStores()) {
				...
				this.dataRepository = new JdbcDataRepository(jdbcUrl, "sa", "");
				this.grants = new JdbcGrantRepository(jdbcUrl, "sa", "");
				this.packages = new JdbcReEncryptedPackageRepository(jdbcUrl, "sa", "");
				...
				this.objectStore = new FileObjectStore(
						Path.of(System.getProperty("rekeyshare.local.objectRoot", "storage/secure-local/objects")));
				this.localKeyProvider = new LocalKeyManagementProvider(Path.of(
						System.getProperty("rekeyshare.local.keyStore", "storage/secure-local/keys/keys.properties")));
			} else {
				this.auditRepository = new InMemoryAuditRepository();
				this.dataRepository = new InMemoryDataRepository();
				this.grants = new InMemoryGrantRepository();
				this.packages = new InMemoryReEncryptedPackageRepository();
				this.idempotency = new IdempotencyService(Duration.ofHours(24));
				this.conversionProofs = new com.example.pre.service.ConversionProofService(
						new com.example.pre.crypto.proof.InMemoryProxySigningKeyRegistry(),
						profile == RuntimeProfile.DEMO
								? new com.example.pre.storage.InMemoryProofReplayRepository()
								: new JdbcProofReplayRepository(System.getProperty(
										"rekeyshare.production.replayJdbcUrl",
										"jdbc:h2:file:./target/runtime/production-proof-replay;AUTO_SERVER=TRUE"), "sa",
										""),
						auditRepository);
				this.objectStore = new InMemoryObjectStore();
				this.localKeyProvider = null;
			}
			this.conversionProofs.assertReplayRepositoryAllowed(profile.name());
		...
		private static String requireSecureLocalSecret() {
			...
			if (secret == null || secret.length() < 24) {
				throw new IllegalStateException(
						"secure-local requires rekeyshare.local.tokenSecret with at least 24 characters");
			}
			return secret;
		}
		...
		RevocationService revocation(AlgorithmType algorithm) {
			return new RevocationService(registry.get(algorithm), dataRepository, grants, packages, objectAuth,
					auditRepository);
		}
		...
		void assertAllowed(String key) {
			...
			synchronized (events) {
				...
				if (events.size() >= maxFailures) {
					throw new ReKeyShareException(ErrorCode.RATE_LIMITED,
							"too many failed authentication or authorization attempts");
				}
			}
		}
