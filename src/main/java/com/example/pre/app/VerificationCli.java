...
public final class VerificationCli {
	...
	private static int verifyEnvelope(PrintStream out) {
		...
		CapsuleContext context = new CapsuleContext("cli-object", "owner", "recipient", AlgorithmType.SECURE_ENVELOPE,
				"owner-key-v1", 1, "cli-policy", "tenant-cli", "grant-cli", "SECURE_ENVELOPE_V1", "proxy-cli",
				"VERIFY_ENVELOPE");
		var capsule = provider.encapsulate(dek, recipient.publicKey(), context);
		boolean recovered = java.util.Arrays.equals(dek,
				provider.decapsulate(capsule, recipient.privateKey(), context));
		boolean tamperRejected = false;
		try {
			CapsuleContext altered = new CapsuleContext("other-object", "owner", "recipient",
					AlgorithmType.SECURE_ENVELOPE, "owner-key-v1", 1, "cli-policy", "tenant-cli", "grant-cli",
					"SECURE_ENVELOPE_V1", "proxy-cli", "VERIFY_ENVELOPE");
			provider.decapsulate(capsule, recipient.privateKey(), altered);
		} catch (IllegalArgumentException expected) {
			tamperRejected = true;
		}
		boolean valid = recovered && tamperRejected;
		out.println("{\"command\":\"crypto verify-envelope\",\"valid\":" + valid + ",\"contextTamperRejected\":"
				+ tamperRejected + "}");
		return valid ? 0 : 1;
	}
	...
	private static int verifyAudit(PrintStream out) {
		...
		out.println("{\"command\":\"audit verify\",\"valid\":" + valid + ",\"chainValid\":" + chainValid
				+ ",\"signatureValid\":" + proofValid + "}");
		return valid ? 0 : 1;
	}
	...
	private static int verifyAttackMatrix(PrintStream out) throws Exception {
		...
		out.println(
				"{\"command\":\"attack-matrix check\",\"valid\":" + valid + ",\"scenarioCount\":" + scenarios + "}");
		return valid ? 0 : 1;
	}
	...
	private static int verifyPackage(PrintStream out) {
		ReEncryptedPackage payload = new ReEncryptedPackage("data-cli", "owner", "recipient", AlgorithmType.RSA_PRE,
				...
				new EncryptedKeyCapsule(AlgorithmType.RSA_PRE, Bytes.utf8("header"), Bytes.utf8("wrapped"),
						Bytes.utf8("keynonce-12b")),
				Instant.now());
		payload = payload.withIssuedManifestHash(PackageManifest.issue(payload).manifestHash());
		SchemeDescriptor descriptor = new SchemeDescriptor("RSA_PRE_BASELINE", "CLI fixture", "EXPERIMENTAL", "test",
		...
		try {
			ReEncryptedPackage changed = new ReEncryptedPackage("data-cli", "owner", "recipient", AlgorithmType.RSA_PRE,
					Bytes.utf8("changed"), Bytes.utf8("nonce-12byte"), Bytes.utf8("aad"), payload.reEncryptedCapsule(),
					Instant.now());
			new PackageVerifier().verify(
					new SharedPackageV2(issued.packageVersion(), issued.schemeId(), issued.parameterSpec(),
							issued.proofStatus(), issued.keyVersion(), issued.expiresAt(), changed, issued.manifest()),
					Instant.now());
			tamperRejected = false;
		} catch (RuntimeException expected) {
			tamperRejected = true;
		}
		out.println("{\"command\":\"verify-package\",\"valid\":" + tamperRejected + ",\"tamperRejected\":"
				+ tamperRejected + "}");
	...
	private static int verifyProof(PrintStream out) throws Exception {
		...
		PolicyBoundConversionProof unsigned = new PolicyBoundConversionProof("POLICY_BOUND_PROOF_V1",
				"POLICY_BOUND_PRE_V1", "tenant-cli", "data-cli", "grant-cli", "owner", "recipient", "proxy-cli",
				"package-cli", "policy", 1, "capsule", "ciphertext", "aad", "manifest", key.keyId(), key.keyEpoch(),
				issuedAt, issuedAt.plusSeconds(900), "nonce-cli", "pending", "Ed25519", "");
		...
		boolean accepted = verifier.verify(proof, issuedAt.plusSeconds(1),
				true) == PolicyBoundProofVerifier.Decision.ACCEPT;
		boolean replayRejected = verifier.verify(proof, issuedAt.plusSeconds(2),
				true) == PolicyBoundProofVerifier.Decision.PROOF_REPLAY_DETECTED;
		boolean valid = accepted && replayRejected;
		out.println("{\"command\":\"verify-proof\",\"valid\":" + valid + ",\"replayRejected\":" + replayRejected + "}");
		return valid ? 0 : 1;
	}
	...
	private static int verifyThreshold(PrintStream out) {
		...
		boolean accepted = java.util.Arrays.equals(secret, service.aggregate(List
				.of(service.convertShare("proxy-a", shares.get(0)), service.convertShare("proxy-b", shares.get(1)))));
		...
		out.println("{\"command\":\"verify-threshold\",\"valid\":" + valid + ",\"insufficientRejected\":"
