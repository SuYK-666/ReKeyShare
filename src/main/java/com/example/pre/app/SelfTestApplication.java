...
public final class SelfTestApplication {
	...
	public static void main(String[] args) throws Exception {
		...
		Files.writeString(Path.of("docs/reports/revocation-test-report.md"), "# Revocation And Rotation Report\n\n"
		...
		Files.writeString(Path.of("docs/reports/audit-chain-report.md"), "# Audit Chain Report\n\n"
	...
	private static void runSuite(PreScheme scheme, List<String> report) {
		...
		var uploaded = data
				.upload(new DataSecurityService.UploadDataCommand(alice, plaintext, "self-test.txt", "text/plain"));
	...
	private static void writeAuditEvidence() throws Exception {
		...
		var uploaded = data
				.upload(new DataSecurityService.UploadDataCommand(alice, plaintext, "audit.txt", "text/plain"));
		...
		Files.writeString(Path.of("docs/reports/audit-verify-result.json"),
				"{\n" + "  \"validResult\": {\n" + "    \"valid\": " + valid.valid() + ",\n" + "    \"checkedEvents\": "
						...
						+ "    \"brokenAt\": \""
						+ (invalid.brokenAt() == null ? "null" : "event-" + String.format("%04d", invalid.brokenAt()))
						+ "\"\n" + "  }\n" + "}\n");
	}
