...
public final class ConcurrencyExperimentApplication {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			for (int limit : new int[]{1, 3, 10}) {
				...
				String grant = field(post(base + "/api/grants", alice,
						"dataId=" + data + "&recipientId=Bob&maxAccessCount=" + limit + "&maxDownloadCount=" + limit)
						.body(), "grantId");
				String dataPackage = field(post(base + "/api/proxy/re-encrypt", proxy, "grantId=" + grant).body(),
						"packageId");
		...
		Files.writeString(summary,
				"# E09 Concurrent Access Limits\n\n" + "- Commit: `"
						+ System.getProperty("rekeyshare.commit", "working-tree") + "`\n" + "- JDK: `"
						...
						+ (allPass ? "PASS" : "FAIL") + "** (no over-issuance).\n");
	}
	...
	private static String createUser(String base, String id, String role) throws Exception {
		return field(post(base + "/api/users", "", "userId=" + id + "&role=" + role + "&algorithm=RSA_PRE").body(),
				"token");
	}
	...
	private static HttpResponse<String> get(String uri, String token) throws Exception {
		return CLIENT.send(
				HttpRequest.newBuilder(URI.create(uri)).header("Authorization", "Bearer " + token).GET().build(),
