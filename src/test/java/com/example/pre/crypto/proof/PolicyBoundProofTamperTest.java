...
class PolicyBoundProofTamperTest {
	...
	void rejectsEveryBoundFieldMutationAndSignerFailure() throws Exception {
		...
		assertEquals(PolicyBoundProofVerifier.Decision.ACCEPT, verifier.verify(valid, issued.plusSeconds(1), false));
	...
	private PolicyBoundConversionProof base(ProxySigningKeyRecord key) {
		return new PolicyBoundConversionProof("POLICY_BOUND_PROOF_V1", "POLICY_BOUND_PRE_V1", "tenant-a", "data-a",
				"grant-a", "owner-a", "recipient-a", "proxy-a", "pkg-a", "policy-a", 7, "capsule-a", "cipher-a",
				"aad-a", "manifest-a", key.keyId(), key.keyEpoch(), issued, issued.plusSeconds(900), "nonce-a",
				"pending", "Ed25519", "");
	}
	...
	private static PolicyBoundConversionProof copy(PolicyBoundConversionProof p, String tenant, String data,
			String grant, String owner, String recipient, String pkg, String policy, int version, String capsule,
			String cipher, String aad) {
	...
	private static PolicyBoundConversionProof withSignature(PolicyBoundConversionProof p, String digest,
		...
		return new PolicyBoundConversionProof(p.proofVersion(), p.algorithmSuite(), p.tenantId(), p.dataId(),
				...
				p.contentKeyVersion(), p.capsuleHash(), p.ciphertextHash(), p.aadHash(), p.manifestHash(), p.keyId(),
				p.keyEpoch(), p.issuedAt(), p.expiresAt(), p.proofNonce(), digest, p.signatureAlgorithm(), signature);
	}
}
