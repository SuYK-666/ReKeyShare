...
class EnvelopeProviderNegativeTest {
    ...
    void validRecipientOpensButWrongRecipientAndContextAreRejected() {
        ...
        var sealed = provider.seal(new HpkeStyleEnvelopeSealRequest(dek, bob.getPublic(), "bob-key-7",
                "tenant-a", "data-a", "bob", "policy-a", 7, Instant.parse("2026-05-26T10:00:00Z")));
    ...
    void tamperedHeaderAndSuiteConfusionAreRejected() {
        ...
        var h = sealed.header();
        var wrongSuite = new HpkeStyleEnvelopeHeader(h.envelopeVersion(), "POLICY_BOUND_PRE_V1", h.kemId(),
                h.kdfId(), h.aeadId(), h.recipientKeyId(), h.tenantId(), h.dataId(), h.recipientId(),
                h.policyHash(), h.contentKeyVersion(), h.createdAt(), h.aadHash());
        var tampered = new HpkeStyleSealedContentKey(wrongSuite, sealed.encapsulatedPublicKey(),
                sealed.ciphertext(), sealed.nonce(), sealed.headerHash());
        assertThrows(IllegalArgumentException.class,
