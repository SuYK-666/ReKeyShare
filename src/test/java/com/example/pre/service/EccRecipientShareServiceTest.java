...
class EccRecipientShareServiceTest {
	...
	void rejectsWrongChallengeAndSignature() {
		...
		var share = DemoPrivateKeyStore.createEccRecipientShareLocally(bob, session.cryptoContext());
		assertThrows(ReKeyShareException.class,
				() -> service.submitRecipientShare(
						new RecipientShareSubmission(session.sessionId(), session.dataId(), session.ownerId(), "Bob",
								"wrong-challenge", share, DemoRecipientShareSignature.shareHash(share),
								DemoRecipientShareSignature.sign(session, share), Instant.now())));
		assertThrows(ReKeyShareException.class,
				() -> service.submitRecipientShare(new RecipientShareSubmission(session.sessionId(), session.dataId(),
						session.ownerId(), "Bob", session.challenge(), share,
						DemoRecipientShareSignature.shareHash(share), "bad-signature", Instant.now())));
	}

	private static RecipientShareSubmission submission(com.example.pre.model.ReKeySession session, String recipientId,
			com.example.pre.crypto.ecc.RecipientReKeyShare share) {
		return new RecipientShareSubmission(session.sessionId(), session.dataId(), session.ownerId(), recipientId,
				session.challenge(), share, DemoRecipientShareSignature.shareHash(share),
				DemoRecipientShareSignature.sign(session, share), Instant.now());
	}
}
